...
async def main():
    async with async_playwright() as pw:
        ...
        # The join sheet is modal by design and fires on the third screen, so on
        # a run that visits every screen it would sit over the rest of them and
        # report three hundred controls as unreachable. It has its own test in
        # test/askprompt.py; here it is answered up front, the way a customer
        # answers it once.
        await pg.evaluate("localStorage.setItem('sp_member_ask_v1', String(Date.now()))")
